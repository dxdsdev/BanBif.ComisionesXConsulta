...
{
    public class SolicitarConsultaController : Controller
    {
        // GET: SolicitarConsulta
        ...
        public ActionResult GuardarConsulta(ConsultaRequest request)
        {
            ConsultaResponse contenidoResponse = new ConsultaResponse();

            try

            {
                string strURL = ConfigurationManager.AppSettings["BaseUrlService"] + "api/ComisionesxConsulta/GuardarConsulta";
                string response = WebApi<ConsultaRequest>.RequestWebApi(request, strURL);
                contenidoResponse = JsonConvert.DeserializeObject<ConsultaResponse>(response);
            }
            catch (Exception ex)
            {
                contenidoResponse.Result = false;
